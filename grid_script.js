//time the update and draw functions to see where time is being spent
...
function update(x, y, lr, reg) {
  ...
  [dW1, dW2, dW3] = tf.tidy(() => {
    ...
    return [tf.add(dW1, tf.mul(reg, W1)), tf.add(dW2, tf.mul(reg, W2)), tf.add(dW3, tf.mul(reg, W3))]
  });
...
function update_and_draw() {
  for(let i = 0; i < 1; i++) {
    update(normalize(predictors), labels, 0.01, 0.003);
  }
  draw();
}
...
function circle_data() {
  ...
  for (let i = 0; i < 64; i++) {
    ...
    predictors.push([x, y, 1]);
    if (Math.pow(x-256, 2) + Math.pow(y-256, 2) > Math.pow(240, 2)) {
      labels.push([1, 0, 0]);
    }else if (Math.pow(x-256, 2) + Math.pow(y-256, 2) > Math.pow(150, 2)) {
      labels.push([0, 1, 0]);
    }else{
      labels.push([0, 0, 1]);
    }
...
const [predictors, labels] = circle_data(100);
